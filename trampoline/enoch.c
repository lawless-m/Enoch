/*
 * Enoch trampoline - WebSocket to 9P bridge for 9front
 *
 * Usage: enoch [-d] [-a addr]
 *   -d      debug mode
 *   -a addr listen address (default: tcp!*!8080)
 *
 * Accepts WebSocket connections on /cpu and bridges them to exportfs.
 */
...
wsread(int fd, WsFrame *f)
{
	...
	if(f->len > 0 && readn(fd, f->data, f->len) != f->len){
		free(f->data);
		return -1;
	}
