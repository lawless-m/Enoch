...
pub fn pass_to_key(password: &str) -> [u8; DESSION] {
    ...
    let mut n = pw_bytes.len().min(ANAMELEN - 1);
    buf[..n].copy_from_slice(&pw_bytes[..n]);
    buf[n] = 0; // null terminate

    // t is our sliding window position in buf
    ...
    loop {
        ...
        for i in 0..DESSION {
            key[i] = (buf[t_offset + i] >> i).wrapping_add(buf[t_offset + i + 1] << (7 - i));
        }
...
mod tests {
    ...
    fn test_pass_to_key_interop() {
        ...
        // Note: The long password case was removed because Nawin.Auth used | instead of +
        // which gives wrong results for passwords > 8 chars. Our implementation now matches
        // 9front's correct algorithm using wrapping_add.
        let test_vectors: &[(&str, &str)] = &[
            ...
            // ("this is a long password", "..."), // Nawin.Auth had a bug here
        ];
